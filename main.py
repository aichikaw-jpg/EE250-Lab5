...
def main():
    ...
    for location in locations:
        ...
        #use a for loop that gets 10 samples
        for i in range(10):
            #get a signal for each data point
            signalSample = get_wifi_signal_strength()
            #add to the signal_strengths list
            signal_strengths.append(signalSample)
            #wait 1 second between each sample
            time.sleep(1)

        
        # TODO: calculate the mean and standard deviation of the signal strengths you collected at this location
        #Get the sum of the signal strength
        signalSum = sum(signal_strengths)
        #get mean by dividing sum by 10
        signal_strength_mean = signalSum / 10

        #initialize signal standard deviations
        signalStdList = []
        #get square deviations
        for x in signal_strengths:
            signalStd = (x - signal_strength_mean) ** 2
            #add the deviation 
            signalStdList.append(signalStd)

        #get the sum of the deviations
        signalStdSum = sum(signalStdList)
        #divide by 10
        signalStdSum /= 10
        #square root 
        signal_strength_std = math.sqrt(signalStdSum)
